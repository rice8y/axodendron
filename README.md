...
![Branch-order analysis before and after pruning the axon](package/images/readme-analysis.png)

This is the actual output of [`package/examples/analysis.typ`](package/examples/analysis.typ): the complete morphology is on the left and the automatically refitted dendritic result is on the right. It uses `AA0109.CNG.swc` from [NeuroMorpho.Org record 85226](https://neuromorpho.org/api/neuron/id/85226), MouseLight, [DOI 10.1002/jnr.23978](https://doi.org/10.1002/jnr.23978) and [DOI 10.25378/janelia.5526706](https://doi.org/10.25378/janelia.5526706), under [CC BY 4.0](https://creativecommons.org/licenses/by/4.0/).
...
The default `color-by: "type"` mapping uses red for soma, green for basal and apical dendrites, and blue for axon; exact fallback and scalar-palette rules, scientific definitions, and the complete public API are collected in the [Axodendron manual](package/docs/documentation.pdf).
...
![The same Vipr2 morphology rendered in XY, XZ, and oblique projections](package/images/readme-rendering.png)

This is the actual output of [`package/examples/rendering.typ`](package/examples/rendering.typ), ordered XY, XZ, and oblique from left to right. It uses `Vipr2-IRES2-Cre_Ai14-310513-05-02-01_637021223_m.CNG.swc` from [NeuroMorpho.Org record 102520](https://neuromorpho.org/api/neuron/id/102520), Allen Cell Types, [DOI 10.1016/j.neuron.2015.02.022](https://doi.org/10.1016/j.neuron.2015.02.022), under [CC BY 4.0](https://creativecommons.org/licenses/by/4.0/).
...
![A CeTZ leader pointing to an exact projected dendrite terminal](package/images/readme-cetz.png)

This is the actual output of [`package/examples/cetz.typ`](package/examples/cetz.typ), using type-3 terminal node 447 from `AA0109.CNG.swc`, [NeuroMorpho.Org record 85226](https://neuromorpho.org/api/neuron/id/85226), MouseLight, [DOI 10.1002/jnr.23978](https://doi.org/10.1002/jnr.23978) and [DOI 10.25378/janelia.5526706](https://doi.org/10.25378/janelia.5526706), under [CC BY 4.0](https://creativecommons.org/licenses/by/4.0/). A leader is straight by default, `via` adds explicit line segments, and one or two `controls` select a quadratic or cubic CeTZ Bezier curve. Curvature and routing are never inferred automatically and should be verified in the final PDF.
...
Typst sources live directly under [`package/examples/`](package/examples/), while all input morphologies live separately under [`package/examples/data/`](package/examples/data/); `.typ` and `.swc` files are never mixed at one directory level.

- [`package/examples/basic.typ`](package/examples/basic.typ) demonstrates analysis, labels, scalar coloring, Sholl analysis, and a two-panel figure with the attributed AA0109 reconstruction.
- [`package/examples/cetz.typ`](package/examples/cetz.typ) uses a routed CeTZ arrow to label an exact projected terminal node in the AA0109 reconstruction.
- [`package/examples/morphometrics.typ`](package/examples/morphometrics.typ) computes a pairwise bifurcation field, explicitly maps it to branch nodes, and renders local angle.
- [`package/examples/overview.typ`](package/examples/overview.typ) lays out three real morphologies in parallel without frames or figure-level decoration and produces the overview image.
- [`package/examples/population.typ`](package/examples/population.typ) creates a three-morphology feature table with explicit field aggregation and missing policy.
- [`package/examples/readme.typ`](package/examples/readme.typ) renders the attributed Sst morphology used by Quick start.
- [`package/examples/analysis.typ`](package/examples/analysis.typ) visualizes branch-order analysis and a pure pruning transformation.
- [`package/examples/rendering.typ`](package/examples/rendering.typ) compares named and arbitrary orthographic projections.
- [`package/examples/tmd.typ`](package/examples/tmd.typ) renders a path-distance TMD barcode and persistence diagram.
- [`package/examples/topology.typ`](package/examples/topology.typ) renders a path-length dendrogram colored by Strahler order.
- [`package/examples/data/`](package/examples/data/) contains the four attributed NeuroMorpho.Org examples listed in [`package/THIRD_PARTY_NOTICES.md`](package/THIRD_PARTY_NOTICES.md).
...
The check performs formatting, warnings-as-errors linting and documentation, normal and adversarial tests, the 250,000-node limit suite, performance budgets, two byte-identical clean WASM builds, package-boundary verification, README synchronization, and Typst smoke compilation. CI additionally recomputes pinned NeuroM 4.0.5 and L-Measure 5.2 cross-validation values and verifies the exact decoded RGBA pixels of a Typst 0.14.2 PNG baseline.
