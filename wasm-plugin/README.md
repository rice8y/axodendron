...
The crates are implementation components and set `publish = false`; nothing in this workspace is released to crates.io. The only shipped binary is the reproducible `package/plugin.wasm` built from the repository root with `./scripts/build-plugin.sh`.
